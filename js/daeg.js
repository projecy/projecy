...
var daeg = {
    sidekick: function(e){
        var p = $('.'+e+'_ads iframe').attr('src');
        $('.'+e+'_ads iframe').attr({src: ''});
        var hide = function(){
            utils.ga($('.'+e).attr('mo'), 'Sidekick Collapse', 'Sidekick');
            $('.'+e+'_sto').animate({'left': -970}, 'slow', function(){
                $('.'+e+'_ads iframe').attr({src: ''});
            });
            $('.'+e+'_masking').animate({'width': 0}, 'slow', function(){$(this).hide()});
        }
        var expand = function(){
            $('.'+e+'_masking').show().animate({'width': 970}, 'slow');
            $('.'+e+'_sto').animate({'left': 0}, 'slow'); 
            $('html, body').animate({scrollLeft: $(window).width()}, 'slow');
        }
        var show = function(event){
            if(event.data.option==0){
                utils.ga($('.'+e).attr('mo'), 'Mouseover Expand', 'Sidekick');
            }else{
                utils.ga($('.'+e).attr('mo'), 'Click Expand', 'Sidekick');
            }
            $('.'+e+'_ads iframe').attr({src: p});
            $('.'+e+'_pointer').off('mouseover');
            setTimeout(expand, 100);
        }
        $('.'+e+'_pointer').on('mouseover', {option: 0}, show);
        $('.'+e+'_pointer').on('click', {option: 1}, show);
	...
	sto_interstitial: function(e){
		var hide = function(){
			$('.'+e).animate({'opacity':0}, 'slow', function(){
				$(this).hide();
				utils.ga($('.'+e).attr('mo'), 'manual hide', e);
			});
		}
		$('.'+e+'_close').on('click', hide);
	},
    ...
    top_frame: function(e) { 
		...
		var a;
		var expand_stoi = function(event){
			$('.'+a).show().delay(100).animate({'opacity':1}, 'slow');
			if(event.data.onclickevent==0){
				$('.'+e+'_fix').off('mouseover');
				utils.ga($('.'+e).attr('mo'), 'auto expand', a);
			}else{
				utils.ga($('.'+e).attr('mo'), 'manual expand', a);
			}
		}
		var remove_stoi = function(){
			$('.'+e).remove();
			$('.'+a).remove();
		}
		if($('.'+e).attr('status')=='sto_interstitial'){
			a = $('.'+e).attr('status');
			$('.'+e+'_fix').on('mouseover', {onclickevent:false}, expand_stoi);
			$('.'+e+'_fix').on('click', {onclickevent:true}, expand_stoi);
			$('.'+e+'_close').on('click', remove_stoi);
		}
	},
